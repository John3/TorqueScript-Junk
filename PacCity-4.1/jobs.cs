function PacCity_JobTest(%applier, %job)
{
    if(%job $= "Cop")
    {
...
function serverCmdJobs(%client, %job)
{
    if(%job $= "")
    {
        messageClient(%client,'','\c4--PacCity Jobs--');
        messageClient(%client,'','\c4Type /getPay to get your paycheck every 5 days.');
