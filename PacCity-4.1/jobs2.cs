function PacCity_JobTestTwo(%applier, %job)
{
    if(%applier.training >= 5)
    {
        ...
        else if(%job $= "MiddleClassPerson")
        {
            if(%applier.training >= 2)
            {
                ...
                %applier.job2 = "MiddleClassPerson";
                %applier.pay2 = 30;
            }
            else
            {
...
function serverCmdJobs2(%client, %job)
{
    if(%job $= "")
    {
        messageClient(%client,'','\c4--PacCity Jobs--');
        messageClient(%client,'','\c4Type /getPayTwo to get your paycheck every 5 days.');
